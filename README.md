...
- 商品：新增/编辑（名称、分类、缩拼、进价、售价、图片）/上架/下架/删除；**缩拼留空自动按名称生成拼音首字母**（多音字取常用读音），可手改；**缩拼冲突时**新增商品自动追加 `_1`/`_2`……直到唯一（`whh` → `whh_1`），重名缩拼的商品图片文件名不再互相覆盖；图片与手机端同一套本地抠图流程（选图 → 抠图 → 旋转/缩放 → 合成 300×300 白底 JPEG，抠图失败回退原图）；模型与 wasm 不打进 exe，统一从手机端托管的 `/m/bgrem/` 加载并用 IndexedDB 缓存，全系统只存一份。
...
CREATE TABLE shop_list (
  ...
  gds_name TEXT,      -- 商品名
  gds_bt_count INTEGER, -- 预留（新增时写 0）
  gds_ck_count INTEGER, -- 预留（新增时写 0）
  gds_jhj INTEGER,    -- 进价
