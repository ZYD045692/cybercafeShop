...
- 商品：新增/编辑（名称、分类、缩拼、进价、售价、图片）/上架/下架/删除；**缩拼留空自动按名称生成拼音首字母**（多音字取常用读音），可手改；**缩拼冲突时**新增商品自动追加 `_1`/`_2`……直到唯一（`whh` → `whh_1`）；图片与手机端同一套本地抠图流程（选图 → 抠图 → 旋转/缩放 → 合成 300×300 白底 JPEG）；模型与 wasm 不打进 exe，统一从手机端托管的 `/m/bgrem/` 加载并用 IndexedDB 缓存，全系统只存一份。图片文件名：手机端按后端唯一化后的最终缩拼命名（互不覆盖）；管理端是先上传图片（按原始缩拼命名）再建商品，两个同名缩拼的商品图片会互相覆盖。
- **图片编辑器（两端同一套，手机端额外支持拍照）**：
  - **流程**：选图（管理端走系统文件框；手机端「📷拍照 / 🖼从相册」）→ 编辑器内画布**立即显示空白 300×300** → 后台抠图去背景，画布上叠加**假进度条**「正在处理图片，首次加载模型需稍等…」（模型库无真实进度回调，进度条为平滑模拟——首次下载模型会多走一会，之后从 IndexedDB 读缓存一闪而过）→ 抠图完成填充透明图 → 可**旋转**（滑杆 ±45° 或直接在图上左右拖动）、**缩放**、**水平翻转**、**重置** → 点「应用」导出 300×300 白底 JPEG。
  - **失败处理**：抠图失败或图片读取失败（损坏/HEIC 等非常规格式）→ 手机端自动回到选图页重新选择；管理端提示「抠图失败，可重新选图」，画布留空白。
  - **「应用」按钮**只在图片处理完成后可用（处理中点应用会提示「商品图还没处理好」），避免导出空白图。
...
CREATE TABLE shop_list (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  gds_number TEXT,    -- 缩拼（拼音首字母，如 whh；留空自动按名称生成；冲突时后端加后缀 _1/_2…）
