...
- **实时同步**：手机端加商品成功后，服务端广播 `product` 事件，管理端商品页收到即刷新——吧台人员正停在商品页时也能看到手机新加的商品（不用手动切页）。
...
| POST | `/api/m/product` | 新增商品（复用 `ProductIn` 字段校验，缩拼冲突自动加后缀 `_1`；成功后向管理端广播 `product` 事件供商品页实时刷新） |
