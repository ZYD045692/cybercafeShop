...
- 管理端数据根目录默认 = exe 所在目录；可用环境变量 **`LSWSHOP_DATA_DIR`** 覆盖（测试隔离用）。

---

## 8. 部署

1. 吧台主机：运行 `dist\cybercafeShop-admin_v0.2.0_setup.exe` 安装（当前用户安装，不需要管理员权限），启动「莱尚网电竞馆点购管理端」一次让目录生成（防火墙弹窗选允许局域网），然后从托盘右键退出。
