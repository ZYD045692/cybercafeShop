<template>
  <section>
    ...
    <el-card shadow="never" style="max-width:760px;margin-bottom:16px">
      <template #header>店铺信息 <span class="hint">（显示在用户端顶部，保存后用户端下次打开页面生效）</span></template>
      <p class="line">店名：
      ...
      <p class="line" style="justify-content:flex-end"><el-button type="primary" plain @click="saveShop">保存店铺信息</el-button></p>
    </el-card>
    <el-card shadow="never" style="max-width:760px;margin-bottom:16px">
      <template #header>收款码 <span class="hint">（点击图片更换，自动处理成 300×300）</span></template>
      <div class="qrs">
        <div v-for="q in [{k:'wechat',n:'微信收款码'},{k:'alipay',n:'支付宝收款码'}]" :key="q.k" class="qr">
          <img :src="qrSrc(q.k)" :title="`点击更换${q.n}`" class="qrpick" @click="qrInputs[q.k].click()">
    ...
    <el-card shadow="never" style="max-width:760px">
      ...
      <p class="line">
        <el-switch v-model="autostart" @change="setAuto" style="margin-right:8px" />开机自动启动
      </p>
      <p class="line">服务端口：<b>{{ port }}</b></p>
      <p class="line">
        语音播报：
...
.qrpick { cursor: pointer; transition: opacity .15s ease; }
.qrpick:hover { opacity: .75; }
