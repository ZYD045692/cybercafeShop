...
- **Rust 侧禁止用 `Command::new(...).output()` 启动控制台子进程**：release 的 GUI 程序（管理端/用户端 exe）没有控制台，启动 ipconfig/ssh-keygen 这类控制台程序会瞬间弹一个空白控制台窗口再消失（实际表现为「切到某个页面闪一个透明窗」），dev 从命令行跑子进程继承控制台故不弹——这就是典型的「dev 正常、release 异常」。确实要调外部命令时用 `creation_flags(CREATE_NO_WINDOW)`。
- **取本机局域网 IP 用枚举网卡（`if-addrs`），别用 UDP 路由法（`admin.rs::lan_ip` 已踩坑改回）**：之前用 `UdpSocket.connect("223.5.5.5:53")` 取「默认路由出口」的 IP，但当网吧客户端开了代理/虚拟网卡（默认路由走 198.18/19 fake-ip）时，拿到的正是要排除的段，退化成 `127.0.0.1` → 添加商品弹窗的二维码写成 127、手机扫不通。`lan_ip()` 现在枚举本机网卡：跳过回环/未指定/链路本地(169.254)/代理fake-ip(198.18/19)，优先取私有段（192.168/10/172.16-31），改这个函数不要回退到「连外网取 IP」。
...
- **手机端加商品后管理端商品页实时刷新**：手机端 `POST /api/m/product`（`mobile.rs::add_product`）成功后 `events.send({type:"product",...})` 广播，管理端 [Products.vue](admin/src/views/Products.vue) 的 `onMounted` 里 `listen('tf-event')` 收到 `payload.type==='product'` 就 `reload()`，卸载时 `unlisten`。改手机端加商品/删商品时记得同步这个事件；管理端商品页有「实时刷新」依赖它（不像订单页有轮询兜底）。
...
- **商品图 URL 版本号用服务端 `pic_t`（图片文件 mtime），前端禁止用 `Date.now()` 当缓存破坏串**：`products` 接口（公开与管理端）给每个图带 `pic_t`，前端 `?t=<pic_t>`——文件不变则 URL 跨启动/跨切页稳定，WebView2 磁盘缓存可命中；管理端/手机端重传图后 mtime 变 → 新 URL → 各端自动拿到新图（现在手机端改图不再需要重挂载商品页）。`serve_file` 带 `ETag`（文件 mtime）+ `Cache-Control: no-cache` + `If-None-Match → 304`；no-cache 保证每次使用先验证，「上传后立刻可见」语义不依赖 URL 变化。曾用 `Products.vue` 的 `imgT=ref(Date.now())`（每次启动/mount 都换 URL → 磁盘缓存全灭），已删，别改回去。
- **管理端 WebView2 内存治理（壳层，见 `admin/src-tauri/src/lib.rs`）**：① 主窗口收托盘/notify 无卡片隐藏时设 MemoryUsageTargetLevel=Low，显示/来单时恢复 Normal（低配吧台机 7×24 挂机省内存；Low 不暂停 JS 定时器，轮询/事件推送不受影响）；② 每日 4:00 只对**当前隐藏**的窗口 `reload()`（JS 堆一天一清；可见窗口永不被打断；notify reload 后自动重拉未处理订单，不丢单）；③ `additional_browser_args("--disk-cache-size=67108864")` 限制 EBWebView 磁盘缓存不无限膨胀。
